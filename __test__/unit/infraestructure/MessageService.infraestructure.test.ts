...
const mockMessageRepository: MessageRepository = {
  saveMessage: jest.fn().mockImplementation(async (message: string, offset: number) => {
    return { success: true, offset, message };
  }),

  retrieveMessages: jest.fn().mockImplementation(async (offset: number, callback: (content: string, id: string) => void) => {
...
describe('MessageService Unit test suite', (): void => {
  ...
  const messageServiceOnConnectionShouldHandleTheEvent = (event: string, _callBack?: Function): void => {
    messageService.run();
  ...
  beforeEach((): void => {
    ...
    mockServer.on.mockImplementation((event: string, callback: (...args: any[]) => any): jest.Mocked<Server> => {
      if (event == 'connection')
  ...
  test(`Test ${number++}: should be able to create a new MessageService and start expecting connections`, (): void => {
    ...
    const ms = new MessageService(mockServer, mockMessageRepository);
    ms.run();
